...
import DashboardViewSingleBooking from './DashboardViewSingleBooking'
import { 
  getPetData, 
  user_delete_pet,
  //clear_state,
...
function DashboardViewBookings(props) {
  ...
  return (
    ...
    {
    	len === 0 ? (<div className={classes.pageTitle}>No bookings yet.</div>) : 
    	(
    	<div>
	      {bookings.slice(0, len).map((booking, i) => (
	         <div>
	          <DashboardViewSingleBooking booking={booking} key={i} fetch_again={fetch_again}/>
	        </div>
	      )).reverse()}</div>
    	)
    }
